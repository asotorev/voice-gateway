"""
S3 Event parsing for Lambda audio processing.

This module handles parsing AWS S3 events that trigger Lambda functions,
...
class S3EventParser:
    ...
    def __init__(self):
        ...
        logger.info("S3 event parser initialized", extra={
    ...
    def get_file_info_summary(self, s3_events: List[Dict[str, Any]]) -> Dict[str, Any]:
        """
        Get summary information for processed S3 events.
        
        Args:
            s3_events: List of parsed S3 events
        ...
        for event in s3_events:
            # Count formats
            extension = self._get_file_extension(event.get('key', ''))
            formats[extension] = formats.get(extension, 0) + 1
            
            # Collect buckets
            buckets.add(event.get('bucket', ''))
        
        return {
            'total_events': len(s3_events),
