"""
AWS configuration manager for Lambda functions.

This module provides centralized AWS client management following Clean Architecture
...
class AWSConfigManager:
    """
    Centralized AWS client management for Lambda functions.
    ...
    def handle_aws_error(self, error: Exception, operation: str, resource: str = "") -> None:
        ...
        if isinstance(error, ClientError):
            ...
            logger.error("AWS ClientError occurred", extra={
                ...
                "aws_request_id": error.response.get('ResponseMetadata', {}).get('RequestId', 'unknown')
            })
        else:
            logger.error("AWS operation failed", extra={
                "operation": operation,
                "resource": resource,
